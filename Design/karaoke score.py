...
def create_karaoke_player():
    ...
    # All buttons now use standard bold font (no underline)
    for tab in ["Home", "Karaoke", "SongBook", "Settings"]:
        tk.Button(nav_frame, text=tab, bg=nav_blue, fg="white", 
                  font=("Courier New", 12, "bold"), bd=0, padx=20, 
                  activebackground="#3d51b3", cursor="hand2").pack(side="left", fill="y")
    ...
    # Blue border frame around the monitor area
    ...
    def style_btn(text, bg_color):
        return tk.Button(btn_container, text=text, font=("Courier New", 12, "bold"), 
                         bg=bg_color, fg="#4A61E1", width=12, pady=10, relief="groove", 
                         bd=1, cursor="hand2")

    style_btn("STOP", "#FFC1C1").pack(pady=5)
    style_btn("ADD", "#E6FFC1").pack(pady=5)
    style_btn("SKIP", "#C1E1FF").pack(pady=5)
